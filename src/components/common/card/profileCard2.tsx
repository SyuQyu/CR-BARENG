...
export default function ProfileCard({
  ...
  const previewText =
    description.slice(0, 305) + (description.length > 305 ? "..." : "");
  ...
  return (
    <div className={clsx("flex flex-col items-start border p-8", className)}>
      <div className="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center w-full">
        <div className="w-1/2 flex flex-row items-center justify-start gap-4">
          ...
          <div className="flex flex-col gap-2">
            <h3 className="md:text-desktop-heading-6 text-mobile-heading-4 font-bold">
              {name}
            </h3>
            <p className="md:text-desktop-caption-l text-mobile-caption-l text-gray-500">
              {location}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col justify-start items-start gap-2 mt-6">
        <p className="md:text-desktop-body-2 text-mobile-body-2">
      ...
      <div className="flex sm:flex-row flex-wrap gap-2  mt-6">
        {tags.map((tag: any, idx: number) => (
          <span
            key={idx}
            className="bg-blue-100 text-blue-700 text-desktop-label-s px-2 py-1 rounded"
          >
            {tag}
          </span>
        ))}
