...
export default function CardReports({
    ...
    return (
        <div className="w-full border-gray-500/20 border-2 group flex flex-col h-full">
            {/* Image container */}
            <div className={clsx("h-full  overflow-hidden", imgType === "1" ? "p-8" : "px-4 pt-4", customImgHeight, imageBorder && "border-b-2 border-gray-500/20")}>
                <ImageWithFallback
                    width={0}
                    height={0}
                    sizes="100vw"
                    className={clsx("w-full transition-transform duration-500 group-hover:-translate-y-2", imgType === "1" ? null : "h-full")}
                    priority={false}
                    src={imageUrl}
                    alt={title}
                />
                ...
                <div className="flex flex-col gap-2">
                    {(badgeLabel || badgeDate) && (
                        <div className="flex items-center gap-2">
                            ...
                            {badgeLabel && badgeDate && (
                                <span className="text-desktop-body-3 font-semibold text-neutral-400">|</span>
                            )}
                            {badgeDate && (
                    ...
                    {title && (
                        <p className="font-bold lg:text-desktop-body-1 text-mobile-body-1">{title}</p>
                    )}
                    {date && (
                        <p className="text-desktop-caption-l text-secondary-foreground/50">{date}</p>
                    )}
                    {country && countryCode && (
                        <span className="inline-flex items-center gap-2 bg-blue-50 rounded-md px-3 py-1 text-black font-medium w-max">
                            <ImageWithFallback
                                ...
                                alt={`${country} Flag`}
                            />
                            <span className="text-desktop-label-s font-semibold tracking-wide">{country}</span>
                        </span>
                    )}
                    {description && (
