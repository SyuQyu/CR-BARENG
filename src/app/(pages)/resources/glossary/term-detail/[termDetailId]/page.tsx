  ...
  return (
    <div>
      ...
      {text.length > maxLength && (
        ...
        >
          {isExpanded ? (
            <p className="flex flex-row text-mobile-body-2">
              Read Less <ChevronUp />
            </p>
          ) : (
            <p className="flex flex-row text-mobile-body-2">
  ...
  return (
    <div className="w-full gap-10 sm:gap-32 flex flex-col justify-center items-center">
      <div className="w-full max-w-[1440px] px-16 lg:px-32 flex flex-col md:gap-14 mb-12">
        ...
        <div className="bg-gray-50 flex flex-col justify-start items-start gap-8 p-8 w-full">
          <div className="flex flex-row justify-start items-center gap-2">
            {DUMMY_TAG?.map((data: any) => (
              <Badge
                key={data.id}
                variant="outline"
                className="text-primary-cr-600 border-primary-cr-600 border-2"
              >
                {data?.name}
              </Badge>
            ))}
          ...
          <div className="flex xl:flex-row flex-col-reverse w-full justify-between gap-8 sm:gap-10 md:gap-16 items-center">
            <div className="flex flex-col gap-[10px]">
              <p className="sm:block hidden text-gray-500 font-semibold sm:text-desktop-body-2 text-desktop-body-3">
                Definition
              </p>
              <p className="sm:block hidden font-bold sm:text-desktop-heading-5 text-mobile-heading-5">
                Athan/Adhan/Azaan
              </p>
              ...
              <div className="md:block hidden">
                <p className="text-mobile-body-2">{definitionText}</p>
              </div>
            </div>
            {layout &&
              (carousel ? (
                <div className="flex flex-col gap-6 w-full px-1.5">
                  <div className="flex flex-col justify-start items-start gap-[5px] w-full">
                    <p className="sm:hidden block text-gray-500 font-semibold sm:text-desktop-body-2 text-desktop-body-3">
                      Definition
                    </p>
                    <p className="sm:hidden block font-bold sm:text-desktop-heading-5 text-mobile-heading-5">
                      Athan/Adhan/Azaan
                    </p>
                  </div>
          ...
          <div className="flex flex-col justify-start items-start gap-4 sm:gap-8 w-full">
            <p className="text-gray-500 font-semibold sm:text-desktop-body-2 text-mobile-heading-6 text-left">
              Related Links
            </p>
      ...
      <div className="w-full px-5 mb-6 sm:mb-16 bg-[#F1F3FF] lg:p-10 flex justify-center items-center pt-6 sm:pt-12">
        <div className="w-full max-w-[1440px] min-h-[460px] flex flex-col gap-10 justify-start items-center">
          <p className="sm:text-desktop-heading-5 text-mobile-heading-4 font-bold sm:mb-4 bg-clip-text text-black text-center">
            Related Terms
          </p>
