...
export default function HotelStep4Page({ params }: { params?: { id: string } }) {
    ...
    // Fetch hotel data if editing (optional, if you want to prefill)
    useEffect(() => {
        if (!params?.id) return;
    ...
    useEffect(() => {
        ...
        if (state.data) {
            ...
            // Next step or finish
            router.push(`/profile/registration/hotel/${params?.id || state.data.id}/finish`);
        } else if (state.error) {
            ...
            <form onSubmit={handleSubmit} ref={formRef} className="w-full mx-auto space-y-6">
                ...
                <div className="flex justify-between items-center pt-4">
                    <Button type="button" asChild>
                        <Link href={`/profile/registration/hotel/${params?.id}/step3`}>
                            Back
                        </Link>
                    </Button>
                    <Button
