...
export default function HotelStep3Page({ params }: { params?: { id: string } }) {
    ...
    useEffect(() => {
        ...
        if (state.data) {
            ...
            router.push(`/profile/registration/hotel/${params?.id || state.data.id}/step4`);
        } else if (state.error) {
    ...
    return (
        <div className="w-full flex flex-col items-start justify-start">
            ...
            <form onSubmit={handleSubmit} ref={formRef} className="w-full mx-auto space-y-6">
                {/* Basic Facilities */}
                <SectionCheckbox
                    title="Internet, Activities and Parking"
                    options={basicFacilitiesOptions}
                    name="basicFacilities"
                    values={formData.basicFacilities}
                    onChange={handleCheckbox}
                />

                {/* Book Now Url */}
                <div className="border p-4 mb-4">
                    <label className="font-bold block mb-2">Book Now Url</label>
                    <Input
                        name="bookNowUrl"
                ...
                <div className="flex justify-between items-center pt-4">
                    <Button type="button" asChild>
                        <Link href={`/profile/registration/hotel/${params?.id}/step2`}>
                            Back
                        </Link>
                    </Button>
